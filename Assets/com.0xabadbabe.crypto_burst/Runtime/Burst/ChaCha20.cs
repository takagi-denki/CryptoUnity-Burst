...
{
    using HChaCha20Burst = HChaCha20;

    [BurstCompile]
    public unsafe struct ChaCha20
    {
        ...
        public static byte[] Encode(byte[] input, byte[] key, byte[] nonce)
        {
            ...
            if (nonce.Length == 24)
            {
                ...
                var hchacha20 = new HChaCha20Burst();
                fixed (byte *k = key, n = hNonce) {
                    HChaCha20Burst.Encode(&hchacha20, k, n);
                }
                for (var i = 0; i < 32; i++) {
                    key[i] = hchacha20.output[i];
                }
            }
