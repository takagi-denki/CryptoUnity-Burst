...
namespace Tests
{
    public class ChaCha20BurstTest
    {
        ...
        public void XChaCha20TestPasses()
        {
            ...
            {
                var result = Crypto.Burst.ChaCha20.Encode(item.input, item.key, item.nonce);
        ...
        public void XChaCha20StreamTestPasses()
        {
            ...
            foreach (var item in list)
            {
                using (var stream = new Crypto.Burst.ChaCha20Stream(item.input, item.key, item.nonce))
                {
        ...
        public void ChaCha20TestSimplePasses()
        {
            ...
            var output = Crypto.Burst.ChaCha20.Encode(message, key, nonce);
        ...
        public void ChaCha20StreamTestSimplePasses()
        {
            ...
            var nonce = StringToByteArray("ddfa69041ecc3feeb077cf45");
            var key = StringToByteArray("2be077349f80bf45faa1f427e81d90dbdc90a1d8d4212c1dacf2bd870000bfdf");
            var stream = new Crypto.Burst.ChaCha20Stream(message, key, nonce);
